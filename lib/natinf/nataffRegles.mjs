/**
 * SIRAL — règles de résolution NATINF → catégorie métier (statistiques).
 *
 * SOURCE UNIQUE de la taxonomie du Mémento parquet et de ses règles de
 * découpage (thème → catégorie, découpages sexuel/écofi par libellé, repli
 * mot-clé, overrides ILE), partagée par :
 *  - l'app web (lib/natinf/nataff.ts, qui apporte le typage et le roll-up
 *    NATAFF N1 pour l'export officiel) ;
 *  - le service attaché (scripts/attache/nataff.mjs), pour que les agrégats
 *    par catégorie des bilans (stats_synthese) donnent EXACTEMENT les mêmes
 *    libellés que la page Statistiques.
 *
 * Tout est éditable à la main ICI, une seule fois : renommer un label,
 * déplacer une catégorie de grand titre, ou ajuster une règle de découpage.
 */
...
const THEME_TO_CAT = Object.fromEntries(
...
function splitSexuel(lib) {
  ...
  if (/\bviols?\b/.test(lib)) return 'VIOL'
...
const KEYWORD_FALLBACK = [
  ...
  // Les infractions ILE sont rattachées par leur code via ILE_CODES ci-dessous.
]
...
/**
 * Code de catégorie métier d'un NATINF ({ code, theme, libelle }), ou
 * undefined si non classé. Overrides par code, puis thème (avec découpage
 * sexuel/écofi), puis repli mot-clé.
 */
export function resolveCategoryCode(entry) {
