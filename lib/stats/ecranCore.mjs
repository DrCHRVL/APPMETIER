 ...
 *
 * Une audience FIXÉE mais pas encore tenue (`isAudiencePending`) n'est PAS une
 * procédure terminée : le dossier est archivé et porte déjà une date
 * d'audience, mais rien n'a encore été jugé. Même règle que le bilan par
 * période (scripts/attache/statistiques.mjs) — sans quoi la carte compte des
 * audiences à venir, y compris au-delà du mois courant, et son total dépasse
 * sa propre ventilation mensuelle. Les dossiers PARTIELLEMENT terminés
 * (`isPartiallyPending`) restent comptés, comme dans la page Archives.
 */
export function proceduresTerminees(resultats, enquetes, annee, maintenant = new Date()) {
  const tous = listeResultats(resultats)

  const enAttenteDAudience = (r) => r?.isAudiencePending === true

  const enquetesTerminees = enquetes.filter((e) => {
    if (e.statut !== 'archive') return false
    const r = resultatDeLEnquete(tous, e.id)
    if (enAttenteDAudience(r)) return false
    return Boolean(r?.dateAudience) && anneeDe(r.dateAudience) === annee
  })
  const directes = tous.filter((r) => r.isDirectResult && !enAttenteDAudience(r)
    && anneeDe(r.dateAudience) === annee)
...
/**
 * Carte « Stupéfiants saisis » : quantités de l'année ventilées par produit,
 * puis par unité d'enquête.
 *
 * Ne portent QUE les saisies de la phase enquête (`resultat.saisies`) — c'est
 * la question posée : combien de produit les services ont-ils sorti du
 * circuit ? Les confiscations prononcées à l'audience sont comptées à part
 * dans `stupefiantsConfisquesParProduit` (mêmes produits des deux côtés :
 * les additionner compterait deux fois la même drogue).
 *
 * ATTENTION à la lecture par service : un dossier CO-SAISI attribue la TOTALITÉ
 * de sa saisie à CHACUN des services concernés (même convention que la carte
 * « Répartition par service », qui compte le dossier une fois par service). La
 * somme des lignes peut donc dépasser le total général, signalé par
 * `coSaisines`.
 *
 * `servicesDe(enquete)` → liste des services de l'enquête (tags « services »).
 */
export function stupefiantsSaisisParService(resultats, enquetes, selecteur, servicesDe) {
  const dans = selecteurResultats(selecteur)
  const retenus = listeResultats(resultats).filter((r) => r?.dateAudience && dans(r))

  const general = totauxStupefiantsVides()
  const parService = new Map()
  const dossiersAvecStups = new Set()
  let coSaisines = 0
  let sansService = 0

  for (const resultat of retenus) {
    const bloc = normaliserStupefiants(resultat.saisies?.stupefiants)
    if (!bloc) continue
    dossiersAvecStups.add(`${resultat.contentieuxId || ''}__${resultat.enqueteId}`)

    for (const produit of bloc.produits) ajouterAuxTotaux(general, produit)

    // Procédure directe (permanence) : le service est porté par le résultat ;
    // sinon on lit les services de l'enquête.
    const enquete = enquetes.find((e) => e.id === resultat.enqueteId)
    const services = (resultat.isDirectResult && resultat.service
      ? [resultat.service]
      : servicesDe(enquete) || []
    ).filter(Boolean)
    ...
    if (services.length > 1) coSaisines++

    for (const service of services) {
      let ligne = parService.get(service)
      if (!ligne) {
        ligne = { service, nbDossiers: 0, totaux: totauxStupefiantsVides() }
        parService.set(service, ligne)
      }
      ligne.nbDossiers++
      for (const produit of bloc.produits) ajouterAuxTotaux(ligne.totaux, produit)
    }
  }

  const lignes = [...parService.values()]
    .sort((a, b) => b.totaux.masseG - a.totaux.masseG
      || b.nbDossiers - a.nbDossiers
      || a.service.localeCompare(b.service, 'fr'))
    .map((l) => ({ ...l, libelle: formatTotaux(l.totaux) }))

  return {
    lignes,
    general: { ...general, libelle: formatTotaux(general) },
    nbDossiers: dossiersAvecStups.size,
    coSaisines,
    sansService,
    aDesDonnees: dossiersAvecStups.size > 0,
    aDesQuantites: !totauxVides(general),
  }
}
...
/**
 * Personnes CONDAMNÉES d'un résultat : les relaxes sont dans `condamnations`
 * (pour garder nom, type d'audience et défèrement) mais n'en font pas partie.
 */
export const condamnesDe = (r) => (r?.condamnations || []).filter((c) => c && !c.isRelaxe)

/** Personnes RELAXÉES d'un résultat. */
export const relaxesDe = (r) => (r?.condamnations || []).filter((c) => c && c.isRelaxe)
...
export function peinesParTypeAudience(resultats, selecteur, types = TYPES_AUDIENCE) {
  ...
  const condamnations = listeResultats(resultats)
    ...
    // Relaxes exclues : ce sont des personnes jugées, pas des condamnations —
    // même population que la carte « Condamnations » (cf. audienceCore).
    .filter((c) => c && !c.isRelaxe)
...
export function interdictionsGererParInfraction(resultats, selecteur, libelleDe = (k) => k, filtreCles = []) {
  ...
  const totalCondamnations = filtres.reduce((n, r) => n + condamnesDe(r).length, 0)
  const totalGerer = filtres.reduce((n, r) => n + condamnesDe(r).filter((c) => c.interdictionGerer).length, 0)
  ...
  for (const r of concernes) {
    ...
    for (const cle of cles.length > 0 ? cles : ['Non renseigné']) {
      ...
      acc[cle].total += condamnesDe(r).length
      acc[cle].gerer += condamnesDe(r).filter((c) => c.interdictionGerer).length
    }
  }
...
/**
 * Carte « Relaxes » : les personnes jugées et NON condamnées de la fenêtre.
 *
 * Une relaxe se compte par PRÉVENU (un dossier peut mêler relaxés et
 * condamnés), au mois de l'audience où elle a été prononcée. Elle ne figure ni
 * dans les condamnations ni dans les moyennes de peine ; `partDesJugesPct` la
 * rapporte à l'ensemble des personnes jugées (condamnés + relaxés), qui est le
 * seul dénominateur où les deux populations coexistent.
 *
 * `libelleDe(cle)` résout un code NATINF en libellé.
 */
export function relaxesDetail(resultats, enquetes, selecteur, libelleDe = (k) => k, options = {}) {
  const { maintenant = new Date() } = options
  const dans = selecteurResultats(selecteur)
  const concernes = listeResultats(resultats).filter((r) => r?.dateAudience && dans(r)
    && !r.isOI && !r.isClassement && !r.isAudiencePending)

  const parMois = serieMois(moisAffiches(typeof selecteur === 'number' ? selecteur : 0, maintenant))
  const comptesInfraction = {}
  const personnes = []
  let condamnes = 0

  for (const r of concernes) {
    condamnes += condamnesDe(r).length
    const relaxes = relaxesDe(r)
    if (relaxes.length === 0) continue
    const e = enquetes.find((x) => x.id === r.enqueteId)
    const dossier = e?.numero || r.numeroAudience || `#${r.enqueteId}`
    const code = r.infractionNatinfCodes?.[0]
    const infraction = (code ? libelleDe(code) : undefined) || r.typeInfraction || 'Non renseigné'
    const m = moisDe(r.dateAudience)
    for (const c of relaxes) {
      if (m != null && m in parMois) parMois[m]++
      comptesInfraction[infraction] = (comptesInfraction[infraction] || 0) + 1
      personnes.push({
        nom: c.nom || 'Inconnu',
        dossier,
        dateAudience: r.dateAudience,
        typeAudience: c.typeAudience,
        defere: Boolean(c.defere),
        infraction,
        natinf: code,
      })
    }
  ...
  const juges = condamnes + total

  return {
    total,
    condamnes,
    juges,
    partDesJugesPct: juges > 0 ? Math.round((total / juges) * 1000) / 10 : 0,
    parMois,
    repartitionParInfraction: Object.entries(comptesInfraction)
      .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0], 'fr'))
      .map(([infraction, count]) => ({ infraction, count })),
    personnes: personnes.sort((a, b) => String(b.dateAudience).localeCompare(String(a.dateAudience))),
  }
}
...
/** Enquêtes terminées retenues par l'onglet : jugées dans l'année, hors
 *  classements, OI et audiences fixées non encore tenues — même définition de
 *  « terminée » que la carte (cf. proceduresTerminees). */
export function enquetesTermineesPourInfractions(resultats, enquetes, annee) {
  ...
  return enquetes.filter((e) => {
    ...
    const r = resultatDeLEnquete(tous, e.id)
    if (!r?.dateAudience) return false
    if (r.isClassement || r.isOI || r.isAudiencePending === true) return false
    return anneeDe(r.dateAudience) === annee
  })
}
