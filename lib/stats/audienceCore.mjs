...
/**
 * Accumulateur des quantités de stupéfiants, produit par produit.
 *
 * Les quantités ne s'additionnent qu'entre unités de même famille (masses
 * ensemble, volumes ensemble, chaque unité de comptage pour elle-même) : un
 * dossier qui pèse 250 g et un autre qui compte 12 plants restent lisibles
 * côte à côte plutôt que fondus dans un total qui ne voudrait rien dire.
 *
 * `nbDossiers` compte les résultats où le produit apparaît, qu'une quantité
 * ait été chiffrée ou non — un produit retenu sans pesée reste un produit saisi.
 */
function creerAgregateurStupefiants() {
  const parProduit = new Map()

  const ajouter = (stupefiants) => {
    const bloc = normaliserStupefiants(stupefiants)
    if (!bloc) return
    for (const produit of bloc.produits) {
      let ligne = parProduit.get(produit.code)
      if (!ligne) {
        const entree = getProduitStupefiant(produit.code)
        ligne = {
          code: produit.code,
          libelle: libelleProduit(produit),
          famille: entree ? entree.famille : 'Autres produits',
          nbDossiers: 0,
          totaux: totauxStupefiantsVides(),
        }
        parProduit.set(produit.code, ligne)
      }
      ligne.nbDossiers++
      ajouterAuxTotaux(ligne.totaux, produit)
    }
  }

  // Tri : d'abord ce qui pèse (les masses parlent au magistrat), puis les
  // volumes, puis les unités comptées — un produit chiffré passe donc toujours
  // devant un produit retenu sans pesée. À égalité : nombre de dossiers, puis
  // alphabet, pour un ordre stable d'un calcul à l'autre.
  const totalComptages = (t) => Object.values(t.comptages || {}).reduce((s, n) => s + n, 0)
  const liste = () =>
    [...parProduit.values()].sort(
      (a, b) =>
        b.totaux.masseG - a.totaux.masseG ||
        b.totaux.volumeMl - a.totaux.volumeMl ||
        totalComptages(b.totaux) - totalComptages(a.totaux) ||
        b.nbDossiers - a.nbDossiers ||
        a.libelle.localeCompare(b.libelle, 'fr'),
    )

  return { ajouter, liste }
}
...
export function calculateAudienceStats(resultats, enquetes) {
  ...
  // Relaxes : personnes jugées et NON condamnées. Comptées à part, jamais dans
  // `totalCondamnations` ni dans les moyennes/taux de peine (leur défèrement,
  // lui, reste compté — la personne a bien été déférée).
  let nombreRelaxes = 0
  ...
  const accumulateSaisies = (saisies) => {
    ...
    stupsSaisis.ajouter(sais.stupefiants)
  ...
  // Quantités de stupéfiants : saisies (phase enquête) et confiscations
  // (audience) restent SÉPARÉES — les mêmes produits figurent souvent des deux
  // côtés, les additionner reviendrait à compter deux fois la même drogue.
  const stupsSaisis = creerAgregateurStupefiants()
  const stupsConfisques = creerAgregateurStupefiants()
  ...
  normalResults.forEach((resultat) => {
    ...
    resultat.condamnations?.forEach((condamnation) => {
      ...
      // Compter chaque CRPC séparément. Une relaxe n'en est jamais une (la CRPC
      // suppose la reconnaissance des faits) : si le drapeau est là, l'affaire
      // a été jugée autrement — on ne gonfle pas les orientations avec elle.
      if (condamnation.typeAudience === 'CRPC-Def' && !condamnation.isRelaxe) {
        nombreCRPC++
      }
      ...
      // Relaxe : rien à agréger côté peines. On sort AVANT `totalCondamnations`
      // pour que la carte Condamnations, les taux et les moyennes gardent la
      // même population (personnes effectivement condamnées).
      if (condamnation.isRelaxe) {
        nombreRelaxes++
        return
      }
    ...
    if (resultat.confiscations) {
      ...
      stupsConfisques.ajouter(conf.stupefiants)
  ...
  const stats = {
    ...
    totalObjets,
    totalStupefiants,
    stupefiantsSaisisParProduit: stupsSaisis.liste(),
    stupefiantsConfisquesParProduit: stupsConfisques.liste(),
