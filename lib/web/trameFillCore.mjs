...
function fillInlineTokens(p, resolve) {
  ...
  if (!segs.length) return { xml: p, orphelin: false }

  const full = segs.map((s) => s.text).join('')
  if (!full.includes('{{')) return { xml: p, orphelin: false }
  ...
  while ((m = re.exec(full))) {
    ...
    if (!name) continue
    touched = true
    const valeur = String(resolve(name))
    if (valeur.trim()) toutesVides = false
    ...
    pieces[si0][j0] = valeur
      .split(/(\n)/)
      .map((piece) => (piece === '\n' ? '</w:t><w:br/><w:t xml:space="preserve">' : escXml(piece)))
      .join('')
  }
  if (!touched) return { xml: p, orphelin: false }
  ...
  out += p.slice(pos) // la queue du paragraphe ferme le dernier run

  // Toutes les balises de la ligne sont restées vides : ce qui subsiste n'est
  // qu'un LIBELLÉ orphelin de la papeterie (« Objet : », « À », « Amiens, le »).
  // Une ligne d'étiquette sans valeur est le symptôme même du document « à
  // moitié rempli » — on la retire. Prudence : seulement s'il s'agit bien d'une
  // étiquette (texte court) et non d'une phrase de la trame qui citerait une balise.
  const reste = pieces.map((a) => a.join('')).join('').replace(/[\s:;,.…«»()"'’\-–—]/g, '')
  return { xml: out, orphelin: toutesVides && reste.length <= 30 }
}
...
export function fillPartXml(xml, vars = {}, opts = {}) {
  ...
  let out = mapParagraphs(xml, (p) => {
    if (soloTokenOf(p)) return p
    const r = fillInlineTokens(p, valueOf)
    if (!r.orphelin) return r.xml
    // Toutes les balises de la ligne sont restées vides : ce qui subsiste n'est
    // qu'une ÉTIQUETTE de la papeterie (« Objet : », « À », « Amiens, le »).
    // Une étiquette sans valeur, c'est le document « à moitié rempli » — on
    // retire la ligne entière, comme le magistrat le ferait à la main.
    const sectPr = sectPrOf((p.match(/<w:pPr>[\s\S]*?<\/w:pPr>/) || [''])[0])
    return sectPr ? `<w:p><w:pPr>${sectPr}</w:pPr></w:p>` : ''
  })
  ...
  return reparerCellulesVides(out)
}

/**
 * Une cellule de tableau DOIT contenir au moins un paragraphe (schéma OOXML).
 * Retirer une étiquette restée vide peut avoir vidé la sienne — dans un en-tête
 * de courrier, les coordonnées sont souvent en tableau. On y repose alors un
 * paragraphe vide, sans quoi Word refuserait d'ouvrir le document.
 */
function reparerCellulesVides(xml) {
  return xml.replace(
    /<w:tc(?:\s[^>]*)?>((?:(?!<w:p[\s/>]).)*?)<\/w:tc>/gs,
    (m, inner) => `<w:tc>${inner}<w:p/></w:tc>`,
  )
}
