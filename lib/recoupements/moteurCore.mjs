// lib/recoupements/moteurCore.mjs
//
// MOTEUR DES RECOUPEMENTS ENTRE DOSSIERS.
//
...
//
// OÙ TOURNE-T-IL ? Sur le SERVEUR, dans le service attaché — seul composant
// qui détienne les clés et puisse donc lire le fonds en entier (chiffrement de
// bout en bout oblige). Il n'a plus ni budget mémoire, ni troncature de pièce,
// ni obligation de rendre la main toutes les 25 ms : les brides que le
// navigateur imposait n'ont plus lieu d'être. Les deux crochets de respiration
// et d'interruption demeurent, pour que le service reste réactif pendant un
// long calcul — ils n'écrêtent aucun signal.

import { mecSortedKey, normalizeMecName, sameMecPersonTokens } from './nomsCore.mjs';
import { extractNames, extractValues, extrait, motsRetenus, normalizeAligned, normalizeLoose } from './valeursCore.mjs';

/**
 * Réglages du moteur — surchargeables au cas par cas.
 *
 * @typedef {object} ReglagesRecoupements
 * @property {number} [maxSignals]  Nombre maximum de signaux rendus (les mieux
 *   notés d'abord).
 * @property {number} [minScore]  Confiance minimale d'un signal rendu.
 * @property {number} [maxOccurrences]  Occurrences conservées par signal (une
 *   par dossier au minimum).
 * @property {number} [maxOccurrencesParDossier]  Occurrences conservées PAR
 *   DOSSIER : l'affichage les regroupe par dossier, et ce qui compte est la
 *   diversité des provenances, pas la répétition d'une même mention.
 * @property {number} [maxDossiersPatronyme]  Au-delà de ce nombre de dossiers,
 *   un patronyme est jugé trop répandu pour dire quoi que ce soit.
 * @property {number} [maxCharsFragment]  Longueur maximale d'un fragment
 *   analysé. `Infinity` = la pièce est lue en entier.
 * @property {() => Promise<void>} [respirer]  Rend la main pendant la lecture
 *   du corpus, pour que le service reste réactif. N'écrête aucun signal.
 * @property {() => boolean} [annule]  Interrompt le calcul en cours.
 * @property {(lus: number, total: number) => void} [avancement]  Avancement de
 *   la lecture du corpus, pour l'affichage d'un état de progression.
 */

// Ces valeurs sont celles du SERVEUR : il lit le fonds en entier.
//
// Elles étaient bien plus basses du temps où le calcul se faisait dans
// l'onglet du magistrat — une pièce tronquée à 300 000 caractères, 200 signaux
// au plus — non par choix d'enquête mais pour empêcher le navigateur de tomber.
// Une pièce n'est plus tronquée, et le plafond de signaux est celui d'un
// affichage raisonnable, pas celui d'une mémoire qui manque.
const DEFAUTS = {
  maxSignals: 2000,
  minScore: 0.4,
  maxOccurrences: 40,
 ...
 */
export function patronymeDe(nom) {
 ...
 */
function chercherNom(norm, mots) {
...
export async function detecterRecoupements(
  ...
  const refs = new Map();
  const personnesDeclarees = new Map(); // dossier → clés MEC
  const index = new Map();
  const mentions = [];
  const fragments = [];
  ...
  const declareesUniques = new Map(); // clé triée → nom affiché
  for (const m of mentions) {
  ...
  for (const fragment of fragments) {
    if (!await souffler()) return null;
    const clesDuDossier = personnesDeclarees.get(fragment.dossierKey) || new Set();
    let norm = null;
  ...
  const signaux = [];
  const paireCouverteParPersonne = new Set(); // `${patronyme}|${dossierA}|${dossierB}`
  ...
  const pairesDejaReliees = new Set();
  ...
  for (const groupe of groupes) {
    ...
    const parDossierMentions = new Map();
