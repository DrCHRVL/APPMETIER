...
import { AIR_PDF_SECTIONS, type AIRPdfOptions } from '@/utils/generateAIRPdf';

interface AIRExportPdfModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (options: AIRPdfOptions) => void;
  isExporting: boolean;
  defaultRedacteur?: string;
}

const PREFS_KEY = 'air_pdf_export_prefs_v1';
...
export const AIRExportPdfModal: React.FC<AIRExportPdfModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  isExporting,
  ...
  const [sections, setSections] = useState<Record<string, boolean>>(() => {
    ...
    for (const s of AIR_PDF_SECTIONS) init[s.key] = persisted[s.key] !== false;
    return init;
  });
  ...
  const selectedCount = useMemo(
    () => AIR_PDF_SECTIONS.filter(s => sections[s.key]).length,
    [sections],
  );
  const allSelected = selectedCount === AIR_PDF_SECTIONS.length;
  ...
  const setAll = (value: boolean) =>
    setSections(() => {
      ...
      for (const s of AIR_PDF_SECTIONS) next[s.key] = value;
      return next;
    });
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-base">
            ...
            Export du rapport AEM
          </DialogTitle>
          <DialogDescription className="text-xs text-gray-500">
            Choisissez les sections à inclure et les mentions d'en-tête, puis générez le rapport PDF.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center justify-between border-t pt-2 mt-1">
          <span className="text-xs font-medium text-gray-700">
            Sections ({selectedCount}/{AIR_PDF_SECTIONS.length})
          </span>
          <button
            type="button"
