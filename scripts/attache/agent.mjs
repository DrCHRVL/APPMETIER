...
export function systemPrompt(keys) {
  ...
  return [
    ...
    '- Les trames préfixées « modele- » sont les MODÈLES que TU as extraits des actes validés du magistrat (étude du corpus) : tu peux les créer et les améliorer (trame_enregistrer, versionné). TOUTE AUTRE trame reste intouchable sans instruction explicite. À la rédaction, une trame déposée par le magistrat PRIME sur un modele- du même type.',
