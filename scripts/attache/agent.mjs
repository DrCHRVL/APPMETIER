...
export function systemPrompt(keys) {
  ...
  return [
    ...
    '- GRAPHIQUES DANS LE DOCUMENT : pour insérer un graphique dans un bilan, place un MARQUEUR seul sur sa ligne, à l\'endroit exact où il illustre le propos : [GRAPHIQUE : nom_du_graphique | du=AAAA-MM-JJ | au=AAAA-MM-JJ] (mêmes noms que stats_graphique, période TOUJOURS précisée pour figer le document). À l\'export PDF/Word, SIRAL remplace automatiquement chaque marqueur par l\'image — celle que tu as regardée, mêmes couleurs. Décris néanmoins la dynamique dans le texte : le document doit rester lisible sans les images.',
